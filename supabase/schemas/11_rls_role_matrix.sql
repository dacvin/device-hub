...
create policy member_read_all     on member for select using (auth.role() = 'authenticated');
...
-- Self-update: only non-privileged columns may change. Role, status, department,
-- reports_to, and invited_by are pinned to the existing row's values via subselect.
-- (Promoting yourself or moving departments requires the admin policy.)
create policy member_update_self on member for update
  using (id = auth.uid())
  with check (
    id = auth.uid()
    and role          = (select m.role          from member m where m.id = auth.uid())
    and status        = (select m.status        from member m where m.id = auth.uid())
    and department_id is not distinct from (select m.department_id from member m where m.id = auth.uid())
    and reports_to    is not distinct from (select m.reports_to    from member m where m.id = auth.uid())
    and invited_by    is not distinct from (select m.invited_by    from member m where m.id = auth.uid())
  );
...
create policy device_read  on device for select using (auth.role() = 'authenticated');
...
create policy device_photo_read on device_photo for select using (auth.role() = 'authenticated');
...
create policy device_document_read on device_document for select using (auth.role() = 'authenticated');
...
create policy department_read        on department for select using (auth.role() = 'authenticated');
...
create policy device_group_read        on device_group for select using (auth.role() = 'authenticated');
...
create policy manufacturer_read        on manufacturer for select using (auth.role() = 'authenticated');
