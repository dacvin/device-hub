    ...
    <div className="min-h-screen w-full grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="relative flex flex-col px-6 py-8 md:px-14 md:py-10 bg-background">
        ...
        <p className="text-[12px] text-muted-foreground">
          By signing in you agree to the Sioux Asia{" "}
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(155deg, #277E69, #1F6F5F 48%, #103A33)",
        }}
      >
        ...
        <div className="relative flex-1 flex flex-col justify-center max-w-md">
          ...
          <h2 className="text-[34px] font-semibold leading-tight">
            Every device,{" "}
            <span style={{ color: "#6FCF97" }}>accounted for.</span>
          </h2>
          <p className="mt-4 text-sm opacity-90 max-w-sm">
            Track laptops, monitors, servers and peripherals from purchase to
            retirement — with warranty alerts and inventory cycles built in.
          </p>

          <div className="mt-8 flex flex-wrap gap-2">
            {["Laptops", "Monitors", "Servers", "Printers"].map((chip) => (
              <span
                key={chip}
        ...
        <div className="relative grid grid-cols-3 gap-6 pt-8 border-t border-white/15">
          <Stat value="1,284" label="Devices tracked" />
          <Stat value="8" label="Departments" />
          <Stat value="98.2%" label="Inventory accuracy" />
        </div>
      </div>
    </div>
  );
