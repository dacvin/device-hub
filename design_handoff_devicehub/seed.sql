-- ============================================================
-- DeviceHub — seed data  (run AFTER schema.sql)
-- Matches schema.sql exactly. Lookups by name → devices reference them
-- via subquery, so you don't hand-manage UUIDs. Idempotent-ish:
-- uses ON CONFLICT (name/code) DO NOTHING so re-running won't duplicate.
--
-- Status note: the mockup used short keys (storage/repair); the schema enum
-- is in-storage / in-repair / in-use / retired — already mapped below.
-- ============================================================
...
-- ---- Devices ----
-- Columns: code, name, group, dept, mfr, model, serial, specs, condition,
--          location, quantity, unit, source, status, import, last_check,
--          cycle, warranty_start, warranty_end
INSERT INTO device (
...
-- ---- Cover photos ----
-- One cover photo per device that has imagery in the mock. Replace the URLs
-- with your Supabase Storage public/signed URLs after uploading the files.
-- sort_order 0 = cover; the UPDATE then points device.cover_photo_id at it.
INSERT INTO device_photo (device_id, url, file_name, sort_order)
SELECT d.id, p.url, p.file_name, 0
FROM (VALUES
  ('DEV-2041-XPS', 'device-photos/DEV-2041-XPS.png', 'DEV-2041-XPS.png'),
  ('DEV-2088-MBP', 'device-photos/DEV-2088-MBP.png', 'DEV-2088-MBP.png'),
  ('DEV-3110-U27', 'device-photos/DEV-3110-U27.png', 'DEV-3110-U27.png'),
  ('DEV-4501-LJ',  'device-photos/DEV-4501-LJ.png',  'DEV-4501-LJ.png'),
  ('DEV-5002-SW',  'device-photos/DEV-5002-SW.png',  'DEV-5002-SW.png'),
  ('DEV-5210-NAS', 'device-photos/DEV-5210-NAS.png', 'DEV-5210-NAS.png'),
  ('DEV-7001-C920','device-photos/DEV-7001-C920.png','DEV-7001-C920.png'),
  ('DEV-3140-S27', 'device-photos/DEV-3140-S27.png', 'DEV-3140-S27.png')
...
-- ============================================================
-- Members & access  (run AFTER migration 003_members.sql)
-- department_id resolved by name subquery; reports_to/invited_by wired
-- in a second pass so order doesn't matter.
-- ============================================================
INSERT INTO member (name, email, role, status, department_id, site, phone, joined_at, last_active_at) VALUES
  ('Anh Tran',   'anh.tran@sioux.asia',   'it_admin', 'active',  (SELECT id FROM department WHERE name='IT'),         'HCMC',  '+84 28 7100 1010', DATE '2021-03-08', now()),
  ('Quan Bui',   'quan.bui@sioux.asia',   'it_admin', 'active',  (SELECT id FROM department WHERE name='IT'),         'HCMC',  '+84 28 7100 1011', DATE '2021-09-14', now() - INTERVAL '2 hours'),
  ('Mai Pham',   'mai.pham@sioux.asia',   'manager',  'active',  (SELECT id FROM department WHERE name='Design'),     'HCMC',  '+84 28 7100 1042', DATE '2022-05-02', now() - INTERVAL '1 day'),
  ('Khoa Le',    'khoa.le@sioux.asia',    'manager',  'active',  (SELECT id FROM department WHERE name='Product'),    'Hanoi', '+84 24 7100 2030', DATE '2022-11-21', now() - INTERVAL '3 days'),
  ('Hoa Nguyen', 'hoa.nguyen@sioux.asia', 'manager',  'active',  (SELECT id FROM department WHERE name='Sales'),      'Hanoi', '+84 24 7100 2031', DATE '2023-01-09', now() - INTERVAL '7 days'),
  ('Phuong Ho',  'phuong.ho@sioux.asia',  'manager',  'active',  (SELECT id FROM department WHERE name='Operations'), 'HCMC',  '+84 28 7100 1050', DATE '2023-04-17', now() - INTERVAL '4 days'),
  ('Tuan Vo',    'tuan.vo@sioux.asia',    'viewer',   'active',  (SELECT id FROM department WHERE name='Finance'),    'HCMC',  '+84 28 7100 1061', DATE '2024-02-26', now() - INTERVAL '14 days'),
  ('Linh Dang',  'linh.dang@sioux.asia',  'viewer',   'invited', (SELECT id FROM department WHERE name='HR'),         'HCMC',  NULL,               NULL,             NULL)
ON CONFLICT (email) DO NOTHING;

-- everyone reports to / was invited by Anh Tran (the seed admin); Phuong manages the two viewers
UPDATE member SET reports_to = (SELECT id FROM member WHERE email='anh.tran@sioux.asia')
  WHERE email IN ('quan.bui@sioux.asia','mai.pham@sioux.asia','khoa.le@sioux.asia','hoa.nguyen@sioux.asia','phuong.ho@sioux.asia');
UPDATE member SET reports_to = (SELECT id FROM member WHERE email='phuong.ho@sioux.asia')
  WHERE email IN ('tuan.vo@sioux.asia','linh.dang@sioux.asia');
UPDATE member SET invited_by = (SELECT id FROM member WHERE email='anh.tran@sioux.asia')
  WHERE email <> 'anh.tran@sioux.asia';

-- ============================================================
-- Workspace settings singleton  (run AFTER migration 004_settings.sql)
-- The INSERT in 004 already seeds defaults; this just stamps updated_by.
-- ============================================================
UPDATE org_settings
SET updated_by = (SELECT id FROM member WHERE email='anh.tran@sioux.asia')
WHERE id = true;

-- ---- Per-user preferences (the seed admin prefers dark mode) ----
INSERT INTO user_preference (user_id, theme, default_device_view, mono_codes)
SELECT id, 'dark', 'table', true FROM member WHERE email='anh.tran@sioux.asia'
